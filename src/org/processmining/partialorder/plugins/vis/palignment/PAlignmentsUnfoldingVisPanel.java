...
public class PAlignmentsUnfoldingVisPanel extends JComponent {
	...
	public PAlignmentsUnfoldingVisPanel(PluginContext context, final PAlignments logReplayResult) {
		this.context = context;
		this.data = logReplayResult;
	...
	private JComponent getLogAlignmentPanel() {
		...
		/*
		 * For each trace that is associate with the alignment res, construct
		 * different graphs because the data dependencies are different
		 */
		//			for (int traceIndex : res.getTraceIndex()) {
		//			for (int i = 0; i < data.getLog().size(); i++) {
		for (Entry<Integer, List<PartialOrderGraph>> graphs : data.getGraphs().entrySet()) {
			for (PartialOrderGraph graph : graphs.getValue()) {
				//					if (count > maxNum) {
				//						break MainLoop;
				//					}
				...
				JComponent comp = getNewPOAlignmentGraphPanel(context, graphs.getKey(), graph);
				if (comp == null) {
					continue;
				}
				tracePanel.add(comp);
	...
	private JComponent getNewPOAlignmentGraphPanel(PluginContext context, int traceIndex, PartialOrderGraph graph) {
		...
		JComponent comp = new PAlignmentGraphPanel(context, graph);
...
//			pAlignment = POAlignmentBuilder.computePAlignmentAndUpdateResult(data.getGraph(), data.getInitialMarking(),
//					data.getFinalMarkings(), data.getEventClasses(), data.getMapping(), data.getXTrace(traceIndex),
//					potrace, traceIndex, pores);
//		}
//		//		}
//		return pAlignment;
//	}
